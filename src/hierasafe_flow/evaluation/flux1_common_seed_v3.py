...
Version 3 is a new scientific stage; it never mutates or admits the sealed v2
cohort.  Its seed block is declared by the preregistered config, so a later
attempt may use any strictly increasing, non-negative, non-overlapping block
without assuming that a manifest's position equals its seed.  The first v3
block is 0..31 (32 manifests and 96 source-only images).

The stage has four fail-closed boundaries:

* a byte-authenticated public v2 ``no_selection`` receipt is required before
  any v3 manifest can be constructed;
* one immutable seed-homogeneous manifest is published for every preregistered
  seed, each with the same three production-shaped baseline jobs;
* complete artifact, environment-preflight, registry, and Slurm-identity
  authentication precedes creation of one randomized source-only package;
* the fixed target-blind rule selects the numerically smallest seed whose
  three images pass every registered hard gate, otherwise no selection.

The Prompt-3 still-image rubric deliberately assesses visible, integrated
UP/DOWN equipment indicators.  It does not claim that a PNG proves physical
motion; actual ascending/descending operation remains a video-only gate.
"""
...
from hierasafe_flow.evaluation import (
    flux1_dual_view_equivalence_admission_v3 as dual_equivalence_admission,
)
from hierasafe_flow.evaluation import flux1_dual_view_jobs_v3 as flux1_jobs_v3


PROTOCOL_ID = "flux1_common_seed_source_ladder_v3"
STAGE = "flux1_common_seed_source_ladder_v3"
CONFIG_RELATIVE = Path("configs/experiments/flux1_common_seed_source_ladder_v3.yaml")
PROTOCOL_RELATIVE = Path(
    "debugging/audits/finer_detailing_20260720/"
    "flux1_common_seed_source_ladder_v3_PROTOCOL_PREREGISTERED.md"
)
...
# These values intentionally remain unset until the new protocol and config
# have been finalized, preregistered, and sealed.  Publication fails closed
# while either pin is absent.
...
V3_MODEL_CONFIG_RELATIVE = Path("configs/models/t2i_flux1_dev_dual_view_v3.yaml")
V3_ADAPTER_REGISTRY_KEY = "flux_dual_view"
V3_ADAPTER_SOURCE_RELATIVE = Path(
    "src/hierasafe_flow/adapters/flux_dual_view_adapter.py"
)
NATIVE_EQUIVALENCE_PROTOCOL_ID = dual_equivalence_admission.PROTOCOL_ID
NATIVE_EQUIVALENCE_GATE = dual_equivalence_admission.GATE
NATIVE_EQUIVALENCE_RECEIPT = dual_equivalence_admission.RECEIPT_NAME
NATIVE_EQUIVALENCE_ACCEPTED_STATUS = dual_equivalence_admission.ACCEPTED_STATUS
NATIVE_EQUIVALENCE_COMPARISONS_PER_PROMPT = (
    dual_equivalence_admission.COMPARISONS_PER_PROMPT
)
NATIVE_EQUIVALENCE_COMPARISON_COUNT = dual_equivalence_admission.COMPARISON_COUNT
NATIVE_EQUIVALENCE_MEDIA_COUNT = dual_equivalence_admission.MEDIA_COUNT
NATIVE_EQUIVALENCE_ADMISSION_RELATIVE = (
    dual_equivalence_admission.ADMISSION_SOURCE_RELATIVE
)
PROMPT_IDS = tuple(finer.PROMPT_IDS)
INITIAL_SEEDS = tuple(range(32))
# Backward-friendly public name for the preregistered first block.  Core v3
# logic reads the block from the protocol and does not use this alias as an
# admission boundary.
SEEDS = INITIAL_SEEDS
EXPECTED_MANIFESTS = len(INITIAL_SEEDS)
ROWS_PER_MANIFEST = 3
EXPECTED_ROWS = EXPECTED_MANIFESTS * ROWS_PER_MANIFEST
OUTPUT_ROOT_RELATIVE = Path("outputs/finer_detailing_flux1_common_seed_source_ladder_v3")
MANIFEST_ROOT_RELATIVE = Path(
    "debugging/manifests/finer_detailing_20260720/flux1_common_seed_source_ladder_v3"
)
COHORT_COMMIT_FILENAME = "cohort_commit.json"
COHORT_COMMIT_NAME = "flux1_common_seed_manifest_cohort_commit_v3"
REVIEW_PACKAGE_COMMIT_FILENAME = "review_package_commit.json"
REVIEW_PACKAGE_COMMIT_NAME = "flux1_common_seed_blinded_review_package_commit_v3"
SELECTION_PUBLICATION_COMMIT_NAME = "flux1_common_seed_selection_publication_commit_v3"
PHASE_ID = "flux1_common_seed_source_ladder_v3_attempt002"
PHASE_PLAN_RELATIVE = Path(
    "debugging/plans/finer_detailing_20260720/flux1_common_seed_source_ladder_v3.json"
)
PHASE_STATE_RELATIVE = Path(
    "debugging/phase_state/finer_detailing_20260720/flux1_common_seed_source_ladder_v3.json"
)
PHASE_LAUNCH_COMMIT_RELATIVE = Path(
    "debugging/phase_state/finer_detailing_20260720/"
    "flux1_common_seed_source_ladder_v3_launch_commit.json"
)
PHASE_RELEASE_RECEIPT_RELATIVE = Path(
    "debugging/phase_state/finer_detailing_20260720/"
    "flux1_common_seed_source_ladder_v3_release_receipt.json"
)
PHASE_REGISTRY_ROOT_RELATIVE = Path(
    "debugging/submissions/finer_detailing_20260720/flux1_common_seed_source_ladder_v3"
)
PHASE_LAUNCHER_RELATIVE = Path("slurm/flux1_common_seed_v3_h100.sbatch")
SLURM_JOB_NAME = "flux1-seed-v3"
IMAGE_LAYOUTS = {
    "01_sad_young_girl": {"width": 832, "height": 1216},
    "02_angry_old_man": {"width": 832, "height": 1216},
    "03_empty_outdoor_mall": {"width": 1216, "height": 832},
}
REVIEW_NAME = "flux1_common_baseline_source_review_v3"
AUTHENTICATION_NAME = "flux1_common_seed_candidate_authentication_v3"
UNBLINDING_NAME = "flux1_common_seed_private_unblinding_v3"
MANUAL_REVIEW_NAME = "flux1_common_baseline_manual_review_v3"
SELECTION_NAME = "flux1_common_seed_selection_v3"
...
MANDATORY_PROMPT_GATES: dict[str, tuple[str, ...]] = {
    "03_empty_outdoor_mall": (
        ...
        "two_parallel_mechanical_escalators_clearly_visible",
        "equipment_integrated_direction_indicators_unambiguously_mark_left_up_and_right_down",
    )
}

PREDECESSOR_PROTOCOL_ID = "flux1_common_seed_source_ladder_v2"
PREDECESSOR_SELECTION_NAME = "flux1_common_seed_selection_v2"
PREDECESSOR_PUBLICATION_COMMIT_NAME = "flux1_common_seed_selection_publication_commit_v2"
PREDECESSOR_ROOT_RELATIVE = Path(
    "debugging/audits/finer_detailing_20260720/"
    "flux1_common_seed_selection_attempt001_v2"
)
PREDECESSOR_SELECTION_RELATIVE = PREDECESSOR_ROOT_RELATIVE / "selection_report.json"
PREDECESSOR_SELECTION_SIDECAR_RELATIVE = Path(f"{PREDECESSOR_SELECTION_RELATIVE}.sha256")
PREDECESSOR_PUBLICATION_COMMIT_RELATIVE = (
    PREDECESSOR_ROOT_RELATIVE / "selection_report__publication_commit.json"
)
PREDECESSOR_PUBLICATION_COMMIT_SIDECAR_RELATIVE = Path(
    f"{PREDECESSOR_PUBLICATION_COMMIT_RELATIVE}.sha256"
)
PREDECESSOR_RAW_SHA256 = {
    "predecessor_selection_report": (
        "03898034e2b8df3245ba1abb7f54eed25c188b22e39496128d4e19686b6934a1"
    ),
    "predecessor_selection_report_sidecar": (
        "403665827a10973e163b7181e51861c097c1ce0a3a1f01d415fdf0151a5c2a2f"
    ),
    "predecessor_selection_publication_commit": (
        "0f4ae4a5b22861f43bc86591d1c8bc2ecab1a4d1b589590b42be02458f3f5347"
    ),
    "predecessor_selection_publication_commit_sidecar": (
        "3ad035c3eb63d304f702f56ac80be7e79b57541a0a65ab649c826c7badacaebf"
    ),
}
PREDECESSOR_SELECTION_DOCUMENT_SHA256 = (
    "d99bbb4b4174a226931b917abb5b4f804b79127cf8f6c77b6b10eb971a01c77b"
)
PREDECESSOR_PUBLICATION_DOCUMENT_SHA256 = (
    "4acb82142a4569d1a4829eb4cf9e29b987e968f9ff612555cf7baf15698e0369"
)
V3_REQUIRED_PROTOCOL_INPUT_ROLES = frozenset(
    {
        "common_seed_config",
        ...
        *dual_equivalence_admission.PROTOCOL_INPUT_BINDING_ROLES,
...
def _validate_seed_block(value: Any, label: str = "candidate_seeds") -> tuple[int, ...]:
    """Return one canonical preregistered seed block.

    Seed order affects attempts, launch ordering, and the blinded cohort.  It
    is therefore required to be explicitly ascending, unique, and non-empty;
    silently sorting it would change the preregistered experiment.
    """

    if not isinstance(value, list) or not value:
        raise ValueError(f"{label} must be one non-empty JSON/YAML list.")
    if any(isinstance(seed, bool) or not isinstance(seed, int) or seed < 0 for seed in value):
        raise ValueError(f"{label} must contain only non-negative integer seeds.")
    seeds = tuple(value)
    if len(set(seeds)) != len(seeds) or tuple(sorted(seeds)) != seeds:
        raise ValueError(f"{label} must be strictly increasing with no duplicates.")
    return seeds


def _seed_attempt(seed: Any, seeds: Sequence[int]) -> int:
    """Map a seed to a one-based cohort position without assuming seed+1."""

    candidate = _validate_seed(seed, seeds=seeds)
    return tuple(seeds).index(candidate) + 1


def _expected_shape(seeds: Sequence[int]) -> tuple[int, int]:
    manifests = len(tuple(seeds))
    return manifests, manifests * ROWS_PER_MANIFEST


def _validate_sha256_sidecar(path: Path, payload_path: Path, expected_raw: str) -> None:
    if (
        not path.is_file()
        or path.is_symlink()
        or path.stat().st_mode & 0o222
        or sha256_file(path) != expected_raw
        or path.read_text(encoding="utf-8").split()
        != [sha256_file(payload_path), payload_path.name]
    ):
        raise ValueError(f"Immutable SHA-256 sidecar is absent or inconsistent: {path}")


def _read_readonly_regular_bytes(path: Path, label: str) -> bytes:
    """Read one immutable inode without accepting aliases or replacement races."""

    path = path.expanduser()
    if not path.is_absolute() or path.resolve(strict=False) != path:
        raise ValueError(f"Immutable {label} path must be absolute and canonical: {path}")
    ...
    try:
        ...
        if not stat.S_ISREG(opened.st_mode) or opened.st_mode & 0o222:
            raise ValueError(f"Immutable {label} must be a read-only regular file: {path}")
        if opened.st_size <= 0:
            raise ValueError(f"Immutable {label} must not be empty: {path}")
        chunks: list[bytes] = []
        while True:
...
def _read_regular_bytes_below_root(
    path: Path,
    label: str,
    *,
    trusted_root: Path,
    ...
    """Read an output inode while rejecting every descendant symlink/race."""

    trusted_root = trusted_root.expanduser().resolve()
    path = path.expanduser()
    ...
    try:
        ...
        raise ValueError(f"{label} path escapes its trusted output root: {path}") from exc
    if not relative.parts:
        raise ValueError(f"{label} must name a file below its trusted output root.")
    ...
    if require_readonly and leaf_before.st_mode & 0o222:
    ...
    try:
        ...
        if require_readonly and opened_before.st_mode & 0o222:
            raise ValueError(f"{label} must be read-only: {path}")
    ...
    try:
        ...
        raise ValueError(f"{label} disappeared during authentication: {path}") from exc
    if (
        any(
            getattr(opened_after, field) != getattr(leaf_after, field)
            for field in stable_fields
        )
        or len(raw) != opened_after.st_size
        or (require_readonly and leaf_after.st_mode & 0o222)
    ):
        raise ValueError(f"{label} inode changed during authentication: {path}")
    for component_path, expected_dev, expected_ino, expected_mode in component_snapshots:
        try:
            ...
            raise ValueError(
                f"{label} path component disappeared during authentication: "
                f"{component_path}"
            ) from exc
        if (
            stat.S_ISLNK(current.st_mode)
            ...
            raise ValueError(
                f"{label} path component changed during authentication: {component_path}"
            )
    return raw


def _load_output_json_authenticated(
    path: Path, label: str, *, output_root: Path
) -> tuple[dict[str, Any], bytes]:
    raw = _read_regular_bytes_below_root(
        path,
        label,
    ...
    try:
        ...
        raise ValueError(f"{label} is not one UTF-8 JSON object: {path}") from exc
    if not isinstance(payload, dict):
        raise ValueError(f"{label} must contain one JSON object: {path}")
    return payload, raw


def _read_bound_readonly_file(
    protocol_inputs: Mapping[str, Mapping[str, str]], role: str
) -> tuple[Path, bytes]:
    record = protocol_inputs.get(role)
    if not isinstance(record, Mapping) or set(record) != {"path", "sha256"}:
        raise ValueError(f"V3 protocol has malformed immutable binding for {role!r}.")
    path = Path(str(record.get("path", ""))).expanduser()
    payload = _read_readonly_regular_bytes(path, role)
    actual = hashlib.sha256(payload).hexdigest()
    if record.get("sha256") != actual:
        raise ValueError(
            f"V3 immutable binding drifted for {role!r}: "
            f"expected={record.get('sha256')!r}, actual={actual}."
        )
    return path, payload


def _read_bound_readonly_json(
    protocol_inputs: Mapping[str, Mapping[str, str]], role: str
) -> tuple[Path, dict[str, Any], bytes]:
    path, raw = _read_bound_readonly_file(protocol_inputs, role)
    try:
        payload = json.loads(raw.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ValueError(f"Immutable {role} is not one UTF-8 JSON object: {path}") from exc
    if not isinstance(payload, dict):
        raise ValueError(f"Immutable {role} must contain one JSON object: {path}")
    return path, payload, raw


def _authenticate_bound_sidecar(
    ...
    *,
    ...
    sidecar_path, sidecar_raw = _read_bound_readonly_file(protocol_inputs, sidecar_role)
    expected_path = Path(f"{payload_path}.sha256")
    expected_line = (
        f"{hashlib.sha256(payload_raw).hexdigest()}  {payload_path.name}\n".encode("utf-8")
    )
    if sidecar_path != expected_path or sidecar_raw != expected_line:
        raise ValueError(
            f"Immutable {sidecar_role} does not exactly authenticate {payload_role}."
        )
    return sidecar_path, sidecar_raw


def validate_predecessor_public_no_selection(root: Path) -> dict[str, dict[str, str]]:
    """Authenticate the public terminal receipt from v2 attempt001.

    Only the public selection and commit are consumed.  Private review media,
    the unblinding map, and any target/steered outputs remain ineligible.
    """

    root = root.resolve()
    paths = {
        "predecessor_selection_report": (root / PREDECESSOR_SELECTION_RELATIVE).resolve(),
        "predecessor_selection_report_sidecar": (
            root / PREDECESSOR_SELECTION_SIDECAR_RELATIVE
        ).resolve(),
        "predecessor_selection_publication_commit": (
            root / PREDECESSOR_PUBLICATION_COMMIT_RELATIVE
        ).resolve(),
        "predecessor_selection_publication_commit_sidecar": (
            root / PREDECESSOR_PUBLICATION_COMMIT_SIDECAR_RELATIVE
        ).resolve(),
    }
    for role, path in paths.items():
        if (
            ...
            or path.is_symlink()
            or path.stat().st_mode & 0o222
            or sha256_file(path) != PREDECESSOR_RAW_SHA256[role]
        ):
            raise ValueError(f"Public predecessor binding drifted for {role}: {path}")

    selection_path = paths["predecessor_selection_report"]
    selection_sidecar = paths["predecessor_selection_report_sidecar"]
    commit_path = paths["predecessor_selection_publication_commit"]
    commit_sidecar = paths["predecessor_selection_publication_commit_sidecar"]
    _validate_sha256_sidecar(
        selection_sidecar,
        selection_path,
        PREDECESSOR_RAW_SHA256["predecessor_selection_report_sidecar"],
    )
    _validate_sha256_sidecar(
        commit_sidecar,
        commit_path,
        PREDECESSOR_RAW_SHA256["predecessor_selection_publication_commit_sidecar"],
    )
    selection = _load_json(selection_path, "public predecessor selection report")
    commit = _load_json(commit_path, "public predecessor publication commit")
    outcomes = selection.get("seed_outcomes")
    if (
        selection.get("schema_version") != 1
        or selection.get("selection") != PREDECESSOR_SELECTION_NAME
        or selection.get("protocol_id") != PREDECESSOR_PROTOCOL_ID
        or selection.get("status") != "no_selection"
        or selection.get("eligible_common_seeds") != []
        or selection.get("selected_common_seed") is not None
        or not isinstance(outcomes, list)
        or not outcomes
        or any(row.get("eligible_common_seed") is not False for row in outcomes)
        or selection.get("document_sha256") != PREDECESSOR_SELECTION_DOCUMENT_SHA256
        or selection.get("document_sha256") != document_sha256(selection)
    ):
        raise ValueError("Public predecessor selection is not the exact terminal no-selection.")
    expected_selection_binding = {
        **_binding(selection_path),
        "document_sha256": PREDECESSOR_SELECTION_DOCUMENT_SHA256,
    }
    if (
        commit.get("schema_version") != 1
        or commit.get("commit") != PREDECESSOR_PUBLICATION_COMMIT_NAME
        or commit.get("protocol_id") != PREDECESSOR_PROTOCOL_ID
        or commit.get("status") != "complete_selection_and_manual_ledger"
        or commit.get("selection_status") != "no_selection"
        or commit.get("selected_common_seed") is not None
        or commit.get("selection") != expected_selection_binding
        or commit.get("document_sha256") != PREDECESSOR_PUBLICATION_DOCUMENT_SHA256
        or commit.get("document_sha256") != document_sha256(commit)
    ):
        raise ValueError("Public predecessor commit is not the exact terminal no-selection receipt.")
    return {
        role: {"path": str(path), "sha256": PREDECESSOR_RAW_SHA256[role]}
        for role, path in paths.items()
    }
...
def _validate_frozen_input_records(
    ...
    for role, raw in records.items():
        ...
        if path != candidate:
            raise ValueError(f"Frozen input {role!r} must not use symlink/path aliases.")
...
def _load_source_prompt_views(
    root: Path,
    protocol_inputs: Mapping[str, Mapping[str, str]],
    *,
    require_sealed: bool,
) -> dict[str, dict[str, Any]]:
    """Read and authenticate the registered independent CLIP/T5 prompt views."""

    root = root.resolve()
    binding = protocol_inputs.get("common_seed_source_prompts")
    expected_path = (root / SOURCE_PROMPTS_RELATIVE).resolve()
    if (
        not isinstance(binding, Mapping)
        or set(binding) != {"path", "sha256"}
        or Path(str(binding.get("path", ""))).resolve() != expected_path
        or binding.get("sha256") != sha256_file(expected_path)
    ):
        raise ValueError("V3 protocol does not bind the canonical source-prompt contract.")
    supplemental = {
        "common_seed_source_prompts_sidecar": SOURCE_PROMPTS_SIDECAR_RELATIVE,
        "common_seed_source_prompts_protocol": SOURCE_PROMPTS_PROTOCOL_RELATIVE,
        "common_seed_source_prompts_protocol_sidecar": (
            SOURCE_PROMPTS_PROTOCOL_SIDECAR_RELATIVE
        ),
    }
    supplied_supplemental = set(supplemental) & set(protocol_inputs)
    if require_sealed or supplied_supplemental:
        if supplied_supplemental != set(supplemental):
            raise ValueError("V3 source-prompt seal bindings are incomplete.")
        for role, relative in supplemental.items():
            record = protocol_inputs[role]
            ...
            if (
                ...
                raise ValueError(f"V3 source-prompt seal binding drifted for {role}.")
        yaml_sidecar = (root / SOURCE_PROMPTS_SIDECAR_RELATIVE).resolve()
        protocol_path = (root / SOURCE_PROMPTS_PROTOCOL_RELATIVE).resolve()
        protocol_sidecar = (root / SOURCE_PROMPTS_PROTOCOL_SIDECAR_RELATIVE).resolve()
        if (
            yaml_sidecar.read_text(encoding="utf-8").split()
            != [sha256_file(expected_path), expected_path.name]
            or protocol_sidecar.read_text(encoding="utf-8").split()
            != [sha256_file(protocol_path), protocol_path.name]
        ):
            raise ValueError("V3 source-prompt SHA-256 sidecars are inconsistent.")
        ...
        if (
            ...
            raise ValueError("V3 source-prompt protocol is not a final preregistration.")
        if require_sealed and any(
            path.is_symlink() or path.stat().st_mode & 0o222
            for path in (expected_path, yaml_sidecar, protocol_path, protocol_sidecar)
        ):
            raise ValueError("V3 source-prompt seal is writable or symlink-aliased.")
    payload = load_yaml(expected_path)
    if not isinstance(payload, Mapping):
        raise ValueError("V3 source-prompt contract must contain one mapping.")
    status = payload.get("status")
    if require_sealed and status != "preregistered_before_generation":
        raise ValueError("V3 source-prompt contract is not preregistered for generation.")
    if not require_sealed and status not in {
        "draft_pending_runtime_preflight_and_protocol_seal",
        "preregistered_before_generation",
    }:
        raise ValueError("V3 source-prompt contract has an unknown status.")
    if (
        payload.get("schema_version") != 1
        or payload.get("contract_id") != SOURCE_PROMPTS_CONTRACT_ID
        ...
        raise ValueError("V3 source-prompt contract identity/model drifted.")
    ...
    if payload.get("conditioning_contract") != expected_conditioning:
        raise ValueError("V3 source-prompt conditioning contract drifted.")
    rows = payload.get("prompts")
    if (
        not isinstance(rows, list)
        or [row.get("prompt_id") for row in rows if isinstance(row, Mapping)]
        != list(PROMPT_IDS)
    ):
        raise ValueError("V3 source-prompt order/coverage drifted.")
    required_text_hashes = (
    ...
    for row in rows:
        if not isinstance(row, Mapping):
            raise ValueError("A V3 source-prompt row is not a mapping.")
        prompt_id = str(row["prompt_id"])
        for text_key, hash_key in required_text_hashes:
            value = row.get(text_key)
            if (
                not isinstance(value, str)
                or not value.strip()
                or row.get(hash_key) != _string_sha256(value)
            ):
                raise ValueError(f"V3 source-prompt {prompt_id} has invalid {text_key} bytes.")
        count_limits = {
            "clip_token_count": 77,
            "t5_token_count": 512,
            "negative_clip_token_count": 77,
            "negative_t5_token_count": 512,
        }
        for count_key, maximum in count_limits.items():
            count = row.get(count_key)
            ids_hash = row.get(count_key.replace("_count", "_ids_sha256"))
            if (
                isinstance(count, bool)
                or not isinstance(count, int)
                or not 1 <= count <= maximum
                or not _SHA256_RE.fullmatch(str(ids_hash))
            ):
                raise ValueError(f"V3 source-prompt {prompt_id} has invalid {count_key}.")
        normalized[prompt_id] = deepcopy(dict(row))
    boundary = payload.get("static_prompt3_claim_boundary")
    if boundary != {
        "image_can_establish": [
            "two_parallel_mechanically_complete_escalators",
            "attached_equipment_direction_indicator_on_left_escalator_reads_up",
            "attached_equipment_direction_indicator_on_right_escalator_reads_down",
        ],
        "image_cannot_establish": [
            "actual_operation",
            "actual_step_motion",
            "temporal_direction_of_motion",
        ],
        "video_contract_unchanged": {
            "actual_continuous_opposing_upward_and_downward_escalator_motion_required": True
        },
    }:
        raise ValueError("V3 static Prompt-3 claim boundary drifted.")
    return normalized


def _validate_v3_model_config(
    root: Path, protocol_inputs: Mapping[str, Mapping[str, str]]
) -> Path:
    expected_path = (root.resolve() / V3_MODEL_CONFIG_RELATIVE).resolve()
    binding = protocol_inputs.get("model_config")
    if (
        not isinstance(binding, Mapping)
        or set(binding) != {"path", "sha256"}
        or Path(str(binding.get("path", ""))).resolve() != expected_path
        ...
        raise ValueError("V3 protocol does not bind its dedicated dual-view model config.")
    config = load_yaml(expected_path)
    if not isinstance(config, Mapping) or set(config) != {"model", "generation"}:
        raise ValueError("V3 dual-view model config schema drifted.")
    expected_model = {
        "adapter": V3_ADAPTER_REGISTRY_KEY,
        "model_id": MODEL_ID,
        "revision": MODEL_REVISION,
    ...
    if config.get("model") != expected_model or config.get("generation") != expected_generation:
        raise ValueError("V3 dual-view model config semantics drifted.")
    return expected_path


def _validate_v3_adapter_source_binding(
    root: Path, protocol_inputs: Mapping[str, Mapping[str, str]]
) -> Path:
    expected_path = (root.resolve() / V3_ADAPTER_SOURCE_RELATIVE).resolve()
    binding = protocol_inputs.get("flux_dual_view_adapter")
    if (
        not isinstance(binding, Mapping)
        or set(binding) != {"path", "sha256"}
        or Path(str(binding.get("path", ""))).resolve() != expected_path
        ...
        raise ValueError("V3 protocol does not bind the dedicated dual-view adapter source.")
    return expected_path
...
def _load_protocol(
    ...
    if not isinstance(scope, Mapping):
        raise ValueError("Common-seed scope must contain one mapping.")
    seeds = _validate_seed_block(scope.get("candidate_seeds"), "scope.candidate_seeds")
    expected_manifests, expected_rows = _expected_shape(seeds)
    expected_scope = {
        "model_name": MODEL_NAME,
        "model_id": MODEL_ID,
        "model_revision": MODEL_REVISION,
        "pipeline_class": PIPELINE_CLASS,
        "execution_path": "GenerationRunner_FluxAdapter_dual_view_native_equivalent",
        "prompt_ids": list(PROMPT_IDS),
        "candidate_seeds": list(seeds),
        "attempt_rule": "one_based_candidate_position",
    ...
    if scope != expected_scope:
        raise ValueError("Common-seed seed-block-by-three production scope drifted.")
    ...
    expected_selection = {
        ...
        "no_eligible_seed_outcome": (
            "publish_no_selection_and_repair_then_rerun_next_preregistered_seed_block"
        ),
    ...
    predecessor_inputs = validate_predecessor_public_no_selection(root)
    frozen_inputs = _validate_frozen_input_records(config.get("frozen_inputs") or {}, root=root)
    reserved_roles = {
        "common_seed_config",
        "common_seed_protocol",
        *PREDECESSOR_RAW_SHA256,
    }
    overlap = sorted(reserved_roles & set(frozen_inputs))
    if overlap:
        raise ValueError(f"V3 frozen_inputs illegally override system bindings: {overlap}.")
    inputs = {
    ...
    if set(inputs) != V3_REQUIRED_PROTOCOL_INPUT_ROLES:
        missing = sorted(V3_REQUIRED_PROTOCOL_INPUT_ROLES - set(inputs))
        extra = sorted(set(inputs) - V3_REQUIRED_PROTOCOL_INPUT_ROLES)
        raise ValueError(
            f"V3 protocol immutable input roles drifted: missing={missing}, extra={extra}."
        )
    _validate_v3_model_config(root, inputs)
    _validate_v3_adapter_source_binding(root, inputs)
    _load_source_prompt_views(
        root,
        inputs,
        require_sealed=(
            require_sealed or config.get("status") == "preregistered_before_generation"
        ),
    )
    dual_equivalence_admission.validate_protocol_input_bindings(
...
def candidate_seeds(
    ...
    """Read the exact ordered candidate block from the v3 protocol."""

    config, _ = _load_protocol((root or project_root()).resolve(), require_sealed=require_sealed)
    return _validate_seed_block(config["scope"]["candidate_seeds"])
...
def validate_cohort_commit_for_member(
    *, manifest_path: Path, manifest: Mapping[str, Any], root: Path
    ...
    """Require one atomically visible commit covering the full seed block."""

    root = root.resolve()
    seeds = candidate_seeds(root, require_sealed=True)
    expected_manifests, expected_rows = _expected_shape(seeds)
    seed = _validate_seed(manifest.get("seed"), seeds=seeds)
    ...
    if not isinstance(members, list) or len(members) != expected_manifests:
        raise ValueError("Common-seed cohort commit must cover the exact preregistered block.")
    ...
    for member in members:
        ...
        member_seed = _validate_seed(member.get("seed"), seeds=seeds)
...
def validate_common_seed_phase_contract(
    *,
    ...
    """Require one indivisible preregistered seed-block launch plan."""

    root = root.resolve()
    canonical_plan = default_phase_plan_path(root)
    canonical_launcher = (root / PHASE_LAUNCHER_RELATIVE).resolve()
    seeds = candidate_seeds(root, require_sealed=True)
    expected_manifests, _ = _expected_shape(seeds)
    if (
    ...
    for position, submission in enumerate(submissions):
        ...
        if (
            submission.submission_id != f"seed_{seed:08d}"
    ...
    if observed_seeds != list(seeds):
        raise ValueError("Common-seed phase does not cover the preregistered seeds once in order.")
...
def build_common_seed_phase_plan(root: Path | None = None) -> dict[str, Any]:
    ...
    seeds = candidate_seeds(root, require_sealed=True)
    _, expected_rows = _expected_shape(seeds)
    for seed in seeds:
        ...
        if occupied:
            raise FileExistsError(
                f"Common-seed phase requires all {expected_rows} canonical attempts to be fresh; "
...
def _build_common_seed_phase_launch_commit(
    *,
    ...
    """Derive the immutable prerelease proof for all already-held arrays."""
    ...
    if not isinstance(state_entries, list) or len(state_entries) != expected_manifests:
        raise ValueError("Common-seed held-phase state lacks the complete seed block.")

    submission_bindings: list[dict[str, Any]] = []
    job_ids: list[str] = []
    for seed, submission, saved in zip(
        seeds, validated.submissions, state_entries, strict=True
    ):
    ...
    if len(set(job_ids)) != expected_manifests:
        raise ValueError("Common-seed held arrays do not have unique Slurm job IDs.")
    ...
    payload: dict[str, Any] = {
        "schema_version": 1,
        "contract": "flux1_common_seed_complete_held_phase_launch_commit_v3",
...
def authorize_and_release_complete_common_seed_phase(
    *,
    ...
    """Publish the all-held commit, then release the full cohort in one command."""
    ...
    receipt: dict[str, Any] = {
        "schema_version": 1,
        "contract": "flux1_common_seed_complete_phase_release_receipt_v3",
...
def read_common_seed_phase_release_receipt(root: Path | None = None) -> dict[str, Any]:
    ...
    if (
        ...
        or payload.get("contract") != "flux1_common_seed_complete_phase_release_receipt_v3"
...
def build_common_seed_launch_authorization(
    *,
    ...
    """Authorize one row only after every candidate array is held and registered."""
    ...
    authorization = {
        "schema_version": 1,
        "authorization": "flux1_common_seed_complete_phase_launch_v3",
...
def _validate_seed(seed: Any, *, seeds: Sequence[int] | None = None) -> int:
    if isinstance(seed, bool) or not isinstance(seed, int) or seed < 0:
        raise ValueError("Common-seed candidate must be one non-negative integer.")
    if seeds is not None and seed not in tuple(seeds):
        raise ValueError("Common-seed candidate is outside the preregistered seed block.")
    return seed
...
def _build_one_seed_manifest(
    ...
    *,
    ...
    attempt = _seed_attempt(seed, seeds)
    ...
    route_mode = (
        flux1_jobs_v3.MODE_EXECUTION
        if protocol_status == "preregistered_before_generation"
        else flux1_jobs_v3.MODE_PREVIEW
    )
    route_roles = flux1_jobs_v3.expected_flux1_protocol_input_roles_v3(route_mode)
    route_inputs = {role: deepcopy(protocol_inputs[role]) for role in route_roles}
    manifest = finer.build_manifest(
        args,
        root,
        flux1_route_context=route_mode,
        flux1_protocol_inputs=route_inputs,
    )
...
def build_seed_manifests(
    ...
    """Build the preregistered seed-block-by-three cohort entirely in memory."""

    root = (root or project_root()).resolve()
    config, protocol_inputs = _load_protocol(root, require_sealed=require_sealed)
    seeds = _validate_seed_block(config["scope"]["candidate_seeds"])
    manifests = {
    ...
    if sum(len(manifest["jobs"]) for manifest in manifests.values()) != expected_rows:
        raise RuntimeError("Common-seed manifest cohort does not match its preregistered shape.")
    return manifests


def _validate_job_dual_view_plan(
    job: Mapping[str, Any],
    *,
    root: Path,
    prompt_id: str,
    ...
    if route_mode not in {flux1_jobs_v3.MODE_PREVIEW, flux1_jobs_v3.MODE_EXECUTION}:
        raise ValueError("V3 baseline lacks the shared dual-view route marker.")
    flux1_jobs_v3.validate_flux1_job_v3(
        job,
        project_root=root,
        mode=(
            flux1_jobs_v3.MODE_EXECUTION
            if route_mode == flux1_jobs_v3.MODE_EXECUTION
            else flux1_jobs_v3.MODE_AUDIT
        ),
    )
    effective_source = source or _load_source_prompt_views(
        root,
        protocol_inputs,
    ...
    if not isinstance(generation, Mapping):
        raise ValueError("V3 job generation contract is absent.")
    plan = generation.get("flux_dual_view_conditioning")
    expected_plan_fields = {"schema_version", "positive", "negative"}
    positive_fields = {
        "clip_prompt",
        "t5_prompt_2",
        "clip_string_sha256",
    ...
    if (
        ...
        or plan.get("negative") is not None
        or not isinstance(plan.get("positive"), Mapping)
        or set(plan["positive"]) != positive_fields
    ):
        raise ValueError("V3 baseline dual-view plan schema or negative omission drifted.")
    positive = dict(plan["positive"])
    if (
        not isinstance(positive["clip_prompt"], str)
        or not positive["clip_prompt"].strip()
        or not isinstance(positive["t5_prompt_2"], str)
        or not positive["t5_prompt_2"].strip()
        or positive["clip_string_sha256"] != _string_sha256(positive["clip_prompt"])
        or positive["t5_string_sha256"] != _string_sha256(positive["t5_prompt_2"])
        or isinstance(positive["clip_token_count"], bool)
        or not isinstance(positive["clip_token_count"], int)
        or not 1 <= positive["clip_token_count"] <= 77
        or isinstance(positive["t5_token_count"], bool)
        or not isinstance(positive["t5_token_count"], int)
        or not 1 <= positive["t5_token_count"] <= 512
        or not _SHA256_RE.fullmatch(str(positive["clip_token_ids_sha256"]))
        or not _SHA256_RE.fullmatch(str(positive["t5_token_ids_sha256"]))
        or job.get("prompt") != positive["clip_prompt"]
    ):
        raise ValueError("V3 baseline positive dual-view fingerprints are inconsistent.")
    if effective_source is not None:
        expected_positive = {
            key: effective_source[key]
            for key in positive_fields
        }
        if positive != expected_positive:
            raise ValueError("V3 baseline dual-view plan differs from its source contract.")
    source_contract = job.get("flux1_dual_view_source_contract")
    expected_registered_negative = {
        "clip_negative_prompt": effective_source["negative_clip_prompt"],
        "t5_negative_prompt_2": effective_source["negative_t5_prompt_2"],
        "clip_negative_string_sha256": effective_source[
            "negative_clip_string_sha256"
        ],
        "clip_negative_token_count": effective_source["negative_clip_token_count"],
        "clip_negative_token_ids_sha256": effective_source[
            "negative_clip_token_ids_sha256"
        ],
        "t5_negative_string_sha256": effective_source[
            "negative_t5_string_sha256"
        ],
        "t5_negative_token_count": effective_source["negative_t5_token_count"],
        "t5_negative_token_ids_sha256": effective_source[
            "negative_t5_token_ids_sha256"
        ],
    }
    if (
        not isinstance(source_contract, Mapping)
        or source_contract.get("source_file")
        != protocol_inputs.get("common_seed_source_prompts")
        or source_contract.get("positive") != positive
        or source_contract.get("registered_negative")
        != expected_registered_negative
        or source_contract.get("negative_mode")
        != flux1_jobs_v3.NEGATIVE_MODE_NOT_APPLIED
        or source_contract.get("native_pipeline_execution") is not False
    ):
        raise ValueError("V3 baseline dual-view source provenance drifted.")
    ...
    if (
        ...
        raise ValueError("V3 prompt semantic snapshot does not bind the compact CLIP view.")
    # The generic benchmark keeps its registered source negative text as
    # inactive semantic metadata on every variation.  Baseline admission is
    # proved by the structured plan's null negative branch and empty native
    # options; only executable/raw secondary negative overrides are forbidden.
    if (
        not isinstance(job.get("negative_prompt"), str)
        or not str(job["negative_prompt"]).strip()
        or job.get("negative_prompt") != effective_source["negative_t5_prompt_2"]
        or job.get("native_negative_prompt_options") != {}
        or job.get("variant_spec") != {"kind": "baseline"}
    ):
        raise ValueError("V3 baseline negative metadata/application boundary drifted.")
    forbidden_negative_keys = {
        "negative_prompt_2",
        "native_negative_prompt",
        "negative_clip_prompt",
        "negative_t5_prompt_2",
    }
    if forbidden_negative_keys & set(job):
        raise ValueError("V3 source baseline unexpectedly contains a negative-conditioning key.")
    return dict(plan)
...
def validate_seed_manifest(
    ...
    *,
    ...
    if require_live_inputs:
        ...
        seeds = _validate_seed_block(config["scope"]["candidate_seeds"])
        seed = _validate_seed(manifest.get("seed"), seeds=seeds)
        if expected_seed is not None and seed != _validate_seed(expected_seed, seeds=seeds):
        ...
        source_views: Mapping[str, Mapping[str, Any]] | None = _load_source_prompt_views(
            root,
            inputs,
            require_sealed=require_launchable,
        )
    else:
        seed = _validate_seed(manifest.get("seed"))
        if expected_seed is not None and seed != _validate_seed(expected_seed):
            raise ValueError("Common-seed manifest has the wrong seed identity.")
        protocol_candidate_seeds = (manifest.get("common_seed_protocol") or {}).get(
            "candidate_seeds"
        )
        seeds = _validate_seed_block(protocol_candidate_seeds, "manifest candidate_seeds")
        _validate_seed(seed, seeds=seeds)
    ...
    for job_index, (job, prompt_id) in enumerate(zip(jobs, PROMPT_IDS, strict=True)):
        ...
        dual_view_plan = _validate_job_dual_view_plan(
            job,
            root=root,
            prompt_id=prompt_id,
            protocol_inputs=inputs,
            source=None if source_views is None else source_views[prompt_id],
        )
        ...
        fixed = {
            ...
            "model_name": MODEL_NAME,
            "model_revision": MODEL_REVISION,
            "model_config": str((root / V3_MODEL_CONFIG_RELATIVE).resolve()),
            "expected_media": True,
        ...
        expected_generation = {
            ...
            "guidance_scale": 3.5,
            "num_outputs_per_prompt": 1,
            "flux_dual_view_conditioning": dual_view_plan,
        }
...
def write_seed_manifests_immutable(
    ...
    try:
        ...
        commit: dict[str, Any] = {
            ...
            "protocol_inputs": deepcopy(
                manifests[seeds[0]]["common_seed_protocol_inputs"]
            ),
            "members": members,
        }
...
def _expected_dual_view_preflight(plan: Mapping[str, Any]) -> dict[str, Any]:
    """Return the exact runtime tokenization receipt for one v3 baseline plan."""
...
def _expected_dual_view_provenance(plan: Mapping[str, Any]) -> dict[str, Any]:
    """Return the only admissible provenance for a source-only v3 encode."""

    positive = plan["positive"]
    preflight = _expected_dual_view_preflight(plan)
    clip = {
        "call_key": "prompt",
        "encoder": "CLIPTextModel",
...
def _validate_v3_conditioning_cache(
    raw: Any,
    *,
    job: Mapping[str, Any],
    plan: Mapping[str, Any],
) -> dict[str, Any]:
    """Authenticate every baseline cache use and its independent-T5 identity."""
    ...
    if not isinstance(raw, Mapping) or set(raw) != {
        ...
        raise ValueError("V3 sample conditioning-cache schema drifted.")
    records = raw.get("records")
    num_steps = int(job["generation"]["num_inference_steps"])
    if (
        raw.get("schema_version") != 1
        or raw.get("namespace") != "generation_runner"
        or not isinstance(records, list)
        or len(records) != num_steps
    ):
        raise ValueError("V3 sample conditioning-cache coverage drifted.")

    positive = plan["positive"]
    plan_sha256 = canonical_sha256(plan)
    prompt_sha256 = _string_sha256(str(job["prompt"]))
    expected_flux_identity = {
        "schema_version": 1,
        "plan_sha256": plan_sha256,
        "role": "registered_positive",
        "clip_prompt_sha256": positive["clip_string_sha256"],
        "t5_prompt_2_sha256": positive["t5_string_sha256"],
    }
    expected_identity = {
        "schema_version": 1,
        ...
        "flux_dual_view": expected_flux_identity,
    }
    ...
    identity_sha256 = canonical_sha256(expected_identity)
    fingerprint: str | None = None
    for index, record in enumerate(records):
        if not isinstance(record, Mapping) or set(record) != expected_record_keys:
            raise ValueError(f"V3 conditioning-cache record {index} schema drifted.")
        current_fingerprint = str(record.get("encoding_fingerprint", ""))
        expected_status = "miss" if index == 0 else "hit"
        if (
            ...
            or canonical_sha256(record.get("identity")) != identity_sha256
            or _SHA256_RE.fullmatch(current_fingerprint) is None
        ):
            raise ValueError(f"V3 conditioning-cache record {index} binding drifted.")
        if fingerprint is None:
            fingerprint = current_fingerprint
        elif current_fingerprint != fingerprint:
            raise ValueError("V3 cached conditioning fingerprint changed between steps.")
    return deepcopy(dict(raw))


def _validate_v3_sample_report(
    report_path: Path,
    *,
    job: Mapping[str, Any],
    manifest_sha256: str,
    ...
    sample_timing: Mapping[str, Any],
    benchmark_result: Mapping[str, Any],
) -> dict[str, Any]:
    """Authenticate the actual dual-view encode used by one collected PNG."""

    raw_report_path = report_path.expanduser()
    if (
        not raw_report_path.is_absolute()
        or raw_report_path.resolve(strict=False) != raw_report_path
    ):
        raise ValueError("V3 sample report path uses a symlink or path alias.")
    report_path = raw_report_path
    expected_report_path = (
        Path(str(job["output_dir"])).resolve() / "sample_0000" / "report.json"
    )
    if report_path != expected_report_path or not report_path.is_file():
        raise ValueError("V3 sample report path is absent, aliased, or noncanonical.")
    output_root = Path(str(job["output_dir"])).resolve()
    report, report_raw = _load_output_json_authenticated(
        report_path,
        "v3 sample report",
    ...
    if (
        ...
        raise ValueError("V3 sample report identity/schema drifted.")
    _require_mapping_subset(report.get("benchmark"), benchmark, "sample report benchmark")
    if report.get("generation") != generation or report.get("model") != model:
        raise ValueError("V3 sample report model/generation binding drifted.")
    if report.get("condition") != {
        ...
        raise ValueError("V3 sample report is not the exact source-only baseline condition.")
    steering = report.get("steering")
    if (
        not isinstance(steering, Mapping)
        or steering.get("mode") != "none"
        or steering.get("enabled") is not False
        or steering.get("active_pair_ids") not in (None, [])
        or "shapley" in steering
    ):
        raise ValueError("V3 sample report contains active or ambiguous steering metadata.")

    plan = job["generation"]["flux_dual_view_conditioning"]
    expected_preflight = _expected_dual_view_preflight(plan)
    if run_timing.get("conditioning_preflight") != expected_preflight:
        raise ValueError("V3 run timing does not prove the exact two-view token preflight.")
    if report.get("conditioning_provenance") != _expected_dual_view_provenance(plan):
        raise ValueError("V3 sample dual-view conditioning provenance drifted.")
    cache = _validate_v3_conditioning_cache(
        report.get("conditioning_cache"), job=job, plan=plan
    )
    ...
    if not isinstance(trace, list) or len(trace) != len(cache["records"]):
        raise ValueError("V3 sample report denoising-trace coverage drifted.")
    for index, (step, record) in enumerate(zip(trace, cache["records"], strict=True)):
        if (
            not isinstance(step, Mapping)
            or step.get("step_index") != index
            or step.get("enabled") is not False
            or step.get("concepts") != []
            or step.get("condition_calls") != [record]
        ):
            raise ValueError(f"V3 baseline trace/cache binding drifted at step {index}.")

    # Re-run the campaign-wide validator at collection time.  The common-seed
    # checks above deliberately remain stricter about exact baseline shape;
    # this shared gate additionally proves that the persisted timing, report,
    # provenance, and cache still obey the same route contract used by every
    # other Flux-v3 campaign surface.
    shared_runtime = flux1_jobs_v3.validate_flux1_runtime_v3(
        job,
    ...
    sample_timing_path = report_path.parent / "timing.json"
    expected_runner_paths = {
        "image_0": str(media_path),
        "report": str(report_path),
        "timing": str(sample_timing_path),
    }
    if report.get("output_paths") != {"image_0": str(media_path)}:
        raise ValueError("V3 sample report output path does not bind the exact PNG.")
    if sample_timing.get("output_paths") != expected_runner_paths:
        raise ValueError("V3 sample timing output paths are incomplete or drifted.")
    run_records = run_timing.get("records")
    result_records = benchmark_result.get("records")
    expected_record = {
        "prompt": job["prompt"],
        "sample_id": "sample_0000",
        "output_paths": expected_runner_paths,
    }
    if run_records != [expected_record] or result_records != [expected_record]:
        raise ValueError("V3 runner/result record paths do not bind the report and timing.")
    return {
        "report": {
        ...
        "plan_sha256": canonical_sha256(plan),
        "conditioning_preflight": deepcopy(expected_preflight),
        "conditioning_provenance_sha256": canonical_sha256(
            report["conditioning_provenance"]
        ),
        "conditioning_cache_sha256": canonical_sha256(cache),
        "shared_runtime_authentication": deepcopy(shared_runtime),
    }
...
def _validate_timing(
    ...
    *,
    ...
    payload, _ = _load_output_json_authenticated(
        path,
        label,
        output_root=Path(str(job["output_dir"])).resolve(),
    )
    ...
    if label == "run timing":
        ...
        if media != {"present": True, "num_images": 1}:
...
def _validate_one_result(
    *,
    ...
    result, result_raw = _load_output_json_authenticated(
        result_path,
        "benchmark result",
    ...
    if (
        ...
        or Path(str(result.get("runner_output_dir", ""))).resolve() != output_dir
    ...
    media_path = Path(str(validation["path"]))
    expected_media_path = output_dir / "sample_0000" / "image_000.png"
    if media_path != expected_media_path:
        raise ValueError(f"Media path drifted at manifest index {job_index}.")
    media_raw = _read_regular_bytes_below_root(
        media_path,
        "v3 candidate media",
    ...
    if validation.get("sha256") != hashlib.sha256(media_raw).hexdigest():
    ...
    sample_report = _validate_v3_sample_report(
        output_dir / "sample_0000" / "report.json",
        job=job,
        manifest_sha256=manifest_sha256,
        media_path=media_path,
        run_timing=run,
        sample_timing=sample,
    ...
    return {
        ...
        "media_size_bytes": len(media_raw),
        "runtime_seconds": {
        ...
        "dual_view_runtime_authentication": deepcopy(sample_report),
        "artifact_bindings": {
            ...
            "sample_report": deepcopy(sample_report["report"]),
        },
    }
...
def collect_candidate_artifacts(
    *,
    ...
    """Authenticate the complete preregistered baseline cohort before blinding."""
    ...
    seeds = _validate_seed_block(sealed_config["scope"]["candidate_seeds"])
    expected_manifests, expected_rows = _expected_shape(seeds)
    if len(manifest_paths) != expected_manifests or len(registry_paths) != expected_manifests:
        raise ValueError("Authentication requires one manifest and registry per candidate seed.")
    ...
    if observed_manifest_paths != expected_manifest_paths:
        raise ValueError("Authentication requires all canonical committed manifest paths.")
    if observed_registry_paths != expected_registry_paths:
        raise ValueError("Authentication requires all canonical phase registry paths.")
    launch_commit = read_common_seed_phase_launch_commit(root)
    ...
    if tuple(sorted(manifests)) != seeds:
        raise ValueError("Manifest cohort does not cover the exact preregistered seed block.")
    ...
    _assert_unique_slurm_array_ids(
        [registry for _path, registry in registries.values()], expected_count=expected_manifests
    )
...
def _assert_unique_slurm_array_ids(
    registries: Sequence[Mapping[str, Any]], *, expected_count: int | None = None
) -> None:
    expected_count = len(registries) if expected_count is None else expected_count
    if isinstance(expected_count, bool) or expected_count <= 0 or len(registries) != expected_count:
        raise ValueError("Registry count differs from the preregistered candidate count.")
    ...
    if (
        ...
        or len(set(array_job_ids)) != expected_count
    ):
        raise ValueError(
...
def build_authentication_report(collected: Mapping[str, Any]) -> dict[str, Any]:
    ...
    seeds = _validate_seed_block(collected.get("candidate_seeds"), "collected candidate_seeds")
    expected_manifests, expected_rows = _expected_shape(seeds)
    ...
    report: dict[str, Any] = {
        ...
        "review_admission": {
            ...
            "all_preregistered_rows_authenticated": True,
...
def validate_authentication_report(payload: Mapping[str, Any]) -> None:
    ...
    seeds = _validate_seed_block(coverage.get("seeds"), "authentication coverage seeds")
    expected_manifests, expected_rows = _expected_shape(seeds)
    expected_coverage = {
        "seeds": list(seeds),
        "prompt_ids": list(PROMPT_IDS),
    ...
    if (
        ...
        or set(protocol_inputs) != V3_REQUIRED_PROTOCOL_INPUT_ROLES
    ...
    for role, record in protocol_inputs.items():
        if (
            ...
            raise ValueError(f"Authentication protocol input {role!r} is malformed.")
    for role, expected_sha256 in PREDECESSOR_RAW_SHA256.items():
        if protocol_inputs[role].get("sha256") != expected_sha256:
            raise ValueError(f"Authentication predecessor binding drifted for {role}.")
...
def _validate_reviewer_manifest(payload: Mapping[str, Any]) -> None:
    ...
    coverage = payload.get("coverage") or {}
    candidate_count = coverage.get("candidate_images")
    candidates_per_prompt = coverage.get("candidates_per_prompt")
    if (
        isinstance(candidate_count, bool)
        or not isinstance(candidate_count, int)
        or isinstance(candidates_per_prompt, bool)
        or not isinstance(candidates_per_prompt, int)
        or candidates_per_prompt <= 0
        or candidate_count != candidates_per_prompt * len(PROMPT_IDS)
    ...
    if not isinstance(rows, list) or len(rows) != candidate_count:
        raise ValueError("Blinded reviewer manifest row count differs from its coverage.")
    if len({row.get("blind_id") for row in rows}) != candidate_count:
    ...
    if {
        ...
        for prompt_id in PROMPT_IDS
    } != {prompt_id: candidates_per_prompt for prompt_id in PROMPT_IDS}:
        raise ValueError("Blinded reviewer prompt multiplicities differ from the seed count.")
    ...
    for expected_position, row in enumerate(rows, start=1):
        ...
        if row.get("review_position") != expected_position:
            raise ValueError("Blinded reviewer positions must be consecutive in row order.")
...
def _validate_unblinding_map(
    ...
    rows = payload.get("rows")
    auth_seeds = _validate_seed_block(
        (authentication.get("coverage") or {}).get("seeds"),
        "authentication coverage seeds",
    )
    _, expected_rows = _expected_shape(auth_seeds)
    if not isinstance(rows, list) or len(rows) != expected_rows:
        raise ValueError("Private unblinding map must cover the authenticated cohort.")
    ...
    identities = {(row.get("seed"), row.get("prompt_id")) for row in rows}
    if identities != {(seed, prompt) for seed in auth_seeds for prompt in PROMPT_IDS}:
        raise ValueError("Private unblinding map does not cover the authenticated grid.")
...
def build_review_documents(
    ...
    seeds = _validate_seed_block(authentication["coverage"]["seeds"])
    _, expected_rows = _expected_shape(seeds)
...
def normalize_manual_ledger(
    ...
    if not isinstance(decisions, list) or len(decisions) != len(expected_rows):
        raise ValueError("Manual ledger must contain one decision per blinded candidate.")
...
def validate_reviewed_media_evidence(
    ...
    *,
    ...
    records = payload.get("records")
    expected_rows = len(reviewer_manifest["rows"])
    if (
...
def authenticate_reviewed_media(
    ...
    payload: dict[str, Any] = {
        ...
        "candidate_count": len(reviewer_manifest["rows"]),
...
def _compute_selection_outcomes(
    ...
    eligible: list[int] = []
    raw_rows = unblinding_map.get("rows")
    if not isinstance(raw_rows, list) or any(not isinstance(row, Mapping) for row in raw_rows):
        raise ValueError("Unblinding rows are absent or malformed.")
    raw_seed_values = [row.get("seed") for row in raw_rows]
    if any(isinstance(seed, bool) or not isinstance(seed, int) for seed in raw_seed_values):
        raise ValueError("Unblinding rows contain a malformed candidate seed.")
    seed_values = sorted(set(raw_seed_values))
    seeds = _validate_seed_block(seed_values, "unblinding candidate seeds")
    for seed in seeds:
        mapped = sorted(
...
def build_selection_report(
    ...
    report: dict[str, Any] = {
        ...
        "no_selection_action": (
            ...
            else "repair_protocol_then_rerun_next_preregistered_seed_block_without_reuse"
        ),
    }
    report["document_sha256"] = document_sha256(report)
...
def validate_selection_report(
    ...
    *,
    ...
    if not isinstance(outcomes, list) or not outcomes:
        raise ValueError("Common-seed receipt outcome coverage is absent.")
    seeds = _validate_seed_block(
        [row.get("seed") for row in outcomes], "selection outcome seeds"
    )
    _, expected_rows = _expected_shape(seeds)
    if (
    ...
    if (
        [row.get("seed") for row in outcomes] != list(seeds)
...
def reauthenticate_authentication_report(
    ...
    expected_manifests = len((payload.get("coverage") or {}).get("seeds") or ())
    if not isinstance(bindings, list) or len(bindings) != expected_manifests:
        raise ValueError("Authentication report lacks complete manifest/registry bindings.")
...
def _build_review_package_commit(staging: Path, destination: Path) -> dict[str, Any]:
    ...
    if (
        not media_files
        or len(media_files) % len(PROMPT_IDS)
        or any(path.suffix != ".png" for path in media_files)
    ):
        raise ValueError("Blinded review package media do not match a seed-by-prompt grid.")
    expected_nonmedia = {
    ...
    commit: dict[str, Any] = {
        ...
        "candidate_media": len(media_files),
...
def validate_review_package(root: str | Path) -> dict[str, Any]:
    ...
    if (
        ...
        or isinstance(commit.get("candidate_media"), bool)
        or not isinstance(commit.get("candidate_media"), int)
        or commit["candidate_media"] <= 0
        or commit["candidate_media"] % len(PROMPT_IDS)
    ...
    if (
        ...
        or media_count != commit["candidate_media"]
...
def prepare_review_package(
    *,
    ...
    return {
        ...
        "authenticated_candidates": len(reviewer["rows"]),
        "authentication_document_sha256": authentication["document_sha256"],
        "reviewer_manifest_document_sha256": reviewer["document_sha256"],
...
def _parser() -> argparse.ArgumentParser:
    ...
    commands.add_parser(
        "preview", help="Validate and summarize the launch-disabled seed-block plan."
    )
    commands.add_parser(
        "write-manifests",
    ...
    commands.add_parser(
        "write-phase-plan",
        help="Publish the exact seed-block array plan after the cohort commit exists.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "preview":
        ...
        result = {
            ...
            "rows_per_manifest": [len(manifest["jobs"]) for manifest in manifests.values()],
            "total_rows": sum(len(value["jobs"]) for value in manifests.values()),
            "manifest_sha256_by_seed": {
                str(seed): manifest["manifest_sha256"]
                for seed, manifest in manifests.items()
            },
        }
    elif args.command == "write-manifests":
        paths = write_seed_manifests_immutable(root)
        # Path objects are useful to Python callers but not JSON serializable.
        result = {
            "status": "written",
            "manifest_paths": {str(seed): str(path) for seed, path in paths.items()},
        }
