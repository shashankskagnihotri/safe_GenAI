...
def _role_pairs(
    ...
    if isinstance(references, _legacy.SwitchReferenceArtifact):
        ...
        try:
            selected = references.references[role]
        except KeyError as exc:
            raise KeyError(
                f"No paired switch references for role {model_role!r}; "
                f"available roles are {sorted(references.references)!r}"
            ) from exc
        if not isinstance(selected, Mapping):
            raise ValueError(f"Malformed paired references for role {role!r}")
        return selected
    ...
    if not isinstance(selected, Mapping):
        selected = root.get("default")
    if not isinstance(selected, Mapping) and len(root) == 1:
