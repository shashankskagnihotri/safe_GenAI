...
def concept_manifest(prompt: dict[str, Any], modality: str, variant: str) -> str:
    prefix = "pilot_concept_manifest" if variant == "current_conceptsteer" else "concept_manifest"
    return str(
        prompt[f"{prefix}_t2v" if modality == "t2v" else f"{prefix}_t2i"]
    )
        ...
        for model in spec["models"]:
            for variant in VARIANTS:
                ...
                row = {
                    ...
                    "calibration_dir": str(
                        Path(spec["campaign"]["calibration_root"])
                        / prompt["prompt_dir"]
                        / model["id"]
                    ),
                }
    ...
    def _condition(self, text: str, state: Any, role: str) -> Any:
        ...
        if key not in self._conditions:
            self._conditions[key] = self.adapter.prepare_prompt_for_state(
                text,
                state,
                prompt_view="prompt_local_22_july",
                call_role=role,
            )
