...
def _scoped_attention_output(
    ...
    *,
    ...
    """Match upstream FLUX routing: preserve text tokens in single blocks."""

    is_flux_single = site_name.startswith(
        ("single_transformer_blocks.", "single_blocks.")
    )
    if image_token_count is None or not is_flux_single:
        return value, 0
    if image_token_count <= 0:
        raise ValueError("MidSteer image_token_count must be positive")
    if value.ndim < 3:
        raise ValueError(
            f"MidSteer FLUX single output requires a sequence axis: {tuple(value.shape)}"
        )
    total_tokens = int(value.shape[-2])
    if image_token_count > total_tokens:
        raise ValueError(
            "MidSteer FLUX image-token count exceeds the single-block sequence: "
...
class MidSteerArtifact:
    ...
    def for_role(self, model_role: str) -> dict[str, list[dict[str, Any]]]:
        ...
        if model_role not in available_roles:
            raise KeyError(
                f"MidSteer has no transforms for exact role={model_role!r}; "
                f"available={sorted(available_roles)!r}"
            )
        result = {
            site: values
            for (candidate_role, site), values in self._index.items()
            if candidate_role == model_role
        }
        if not result:
            raise KeyError(
