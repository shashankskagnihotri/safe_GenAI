#!/usr/bin/env bash
# Fail-closed environment dispatch for the sealed Flux.1 45-row calibration.
...
MANIFEST="${1:?Usage: run_flux1_native_negative_calibration_dispatched.sh MANIFEST INDEX}"
INDEX="${2:?Usage: run_flux1_native_negative_calibration_dispatched.sh MANIFEST INDEX}"
if [ "$#" -ne 2 ]; then
...
if [ -f "${HOME}/miniconda3/etc/profile.d/conda.sh" ]; then
  ...
  echo "conda.sh not found; authenticated calibration dispatch is impossible." >&2
  exit 1
fi
...
if [[ "${DISPATCH_RECORD}" == *$'\n'* ]]; then
  echo "Authenticated calibration dispatcher returned more than one record." >&2
  exit 1
fi
...
if [ -z "${TARGET_ENVIRONMENT}" ] || \
  ...
  echo "Malformed authenticated calibration dispatch record." >&2
  exit 1
fi
...
exec python scripts/run_flux1_native_negative_calibration.py \
  --manifest "${MANIFEST}" \
  --index "${INDEX}"
