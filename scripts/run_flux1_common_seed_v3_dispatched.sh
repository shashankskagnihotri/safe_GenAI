#!/usr/bin/env bash
# Fail-closed dispatcher for one row of the complete Flux common-seed phase.
...
MANIFEST="${1:?Usage: run_flux1_common_seed_v3_dispatched.sh MANIFEST INDEX PHASE_PLAN}"
INDEX="${2:?Usage: run_flux1_common_seed_v3_dispatched.sh MANIFEST INDEX PHASE_PLAN}"
PHASE_PLAN="${3:?Usage: run_flux1_common_seed_v3_dispatched.sh MANIFEST INDEX PHASE_PLAN}"
if [ "$#" -ne 3 ]; then
  echo "Expected exactly MANIFEST, INDEX, and PHASE_PLAN; got $# arguments." >&2
  exit 2
fi
...
if [ -f "${HOME}/miniconda3/etc/profile.d/conda.sh" ]; then
  ...
  echo "conda.sh not found; authenticated common-seed dispatch is impossible." >&2
  exit 1
fi
...
DISPATCH_RECORD="$(
  python scripts/finer_detailing_environment_dispatch_v3.py resolve \
...
if [[ "${DISPATCH_RECORD}" == *$'\n'* ]]; then
  echo "Authenticated common-seed dispatcher returned more than one record." >&2
  exit 1
fi
...
if [ -z "${TARGET_ENVIRONMENT}" ] || \
  ...
  echo "Malformed authenticated common-seed dispatch record." >&2
  exit 1
fi
...
python scripts/finer_detailing_environment_dispatch_v3.py verify-job \
  --phase-plan "${PHASE_PLAN}" \
  --manifest "${MANIFEST}" \
  --index "${INDEX}" \
  --project-root "${PROJECT_ROOT}" \
