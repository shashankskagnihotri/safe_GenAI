#!/usr/bin/env bash
# Dedicated fail-closed launcher for authenticated production-shaped smokes.
...
PLAN="${1:?Usage: run_finer_detailing_smoke_dispatched.sh PLAN ROLE INDEX REGISTRY}"
ROLE="${2:?Usage: run_finer_detailing_smoke_dispatched.sh PLAN ROLE INDEX REGISTRY}"
INDEX="${3:?Usage: run_finer_detailing_smoke_dispatched.sh PLAN ROLE INDEX REGISTRY}"
REGISTRY="${4:?Usage: run_finer_detailing_smoke_dispatched.sh PLAN ROLE INDEX REGISTRY}"
if [ "$#" -ne 4 ]; then
  echo "Expected exactly PLAN ROLE INDEX REGISTRY; got $# arguments." >&2
  exit 2
fi
...
if ! [[ "${SLURM_ARRAY_JOB_ID:-}" =~ ^[0-9]+$ ]] || \
  ...
  ! [[ "${SLURM_JOB_ID:-}" =~ ^[0-9]+$ ]] || \
  [ "${SLURM_JOB_NAME:-}" != "finer-smoke" ]; then
  echo "Dedicated smoke launch requires exact live Slurm array/job identity." >&2
  exit 1
fi
...
if [ -f "${HOME}/miniconda3/etc/profile.d/conda.sh" ]; then
  ...
  echo "conda.sh not found; authenticated smoke dispatch is impossible." >&2
  exit 1
fi
...
DISPATCH_RECORD="$(
  python scripts/finer_detailing_smoke_dispatch.py resolve \
    --plan "${PLAN}" --role "${ROLE}" --index "${INDEX}" \
    --submission-registry "${REGISTRY}" --project-root "${PROJECT_ROOT}"
)"
if [[ "${DISPATCH_RECORD}" == *$'\n'* ]]; then
  echo "Smoke dispatcher returned more than one resolve record." >&2
  exit 1
fi
...
if [ -z "${TARGET_ENVIRONMENT}" ] || [ -z "${AUTHENTICATED_MODEL}" ] || \
  ...
  echo "Malformed authenticated smoke dispatch record." >&2
  exit 1
fi
...
python scripts/finer_detailing_smoke_dispatch.py verify-job \
  --plan "${PLAN}" --role "${ROLE}" --index "${INDEX}" \
  --submission-registry "${REGISTRY}" --project-root "${PROJECT_ROOT}" \
...
export HIERASAFE_REQUIRE_SMOKE_LAUNCH_AUTHORIZATION=1
