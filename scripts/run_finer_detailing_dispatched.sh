#!/usr/bin/env bash
# Fail-closed environment dispatcher shared by both finer-detailing launchers.
...
MANIFEST="${1:?Usage: run_finer_detailing_dispatched.sh MANIFEST INDEX}"
INDEX="${2:?Usage: run_finer_detailing_dispatched.sh MANIFEST INDEX}"
...
if [ -f "${HOME}/miniconda3/etc/profile.d/conda.sh" ]; then
  ...
  echo "conda.sh not found; authenticated environment dispatch is impossible." >&2
  exit 1
fi
...
# The main environment is a bootstrap only for strict manifest authentication.
# It is not assumed to be the generation environment for the selected model.
if ! _activate_conda_environment_nounset_safe safe_genai_conceptsteer; then
  echo "Bootstrap conda env safe_genai_conceptsteer is absent or cannot be activated." >&2
  exit 1
fi
...
if [[ "${DISPATCH_RECORD}" == *$'\n'* ]]; then
  echo "Authenticated environment dispatcher returned more than one record." >&2
  exit 1
fi
...
if [ -z "${TARGET_ENVIRONMENT}" ] || \
  ...
  echo "Malformed authenticated environment dispatch record." >&2
  exit 1
fi

# Activation failure is terminal.  In particular, an LTX job must never fall
# back to the Hunyuan-pinned main environment.
...
# The successful verifier has atomically published the attempt-bound preflight.
# Make its presence and binding mandatory when the generation entry point reopens
# the manifest, closing the verifier-to-model-load race.
